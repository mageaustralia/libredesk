...
DROP TYPE IF EXISTS "message_status" CASCADE; CREATE TYPE "message_status" AS ENUM ('received','sent','failed','pending','sending');
...
DROP TYPE IF EXISTS "user_notification_type" CASCADE; CREATE TYPE "user_notification_type" AS ENUM ('mention', 'assignment', 'sla_warning', 'sla_breach', 'send_failure', 'new_reply');
DROP TYPE IF EXISTS "conversation_status_category" CASCADE; CREATE TYPE "conversation_status_category" AS ENUM ('open', 'waiting', 'resolved', 'spam', 'trashed');
...
CREATE TABLE conversations (
    ...
    merged_into_conversation_id BIGINT REFERENCES conversations(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS index_conversations_merged_into ON conversations (merged_into_conversation_id) WHERE merged_into_conversation_id IS NOT NULL;
...
DROP TABLE IF EXISTS macro_user_usage CASCADE;
CREATE TABLE macro_user_usage (
...
CREATE INDEX idx_macro_user_usage_user_recent ON macro_user_usage(user_id, last_used_at DESC);
