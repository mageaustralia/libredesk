...
func initHandlers(g *fastglue.Fastglue, hub *ws.Hub) {
	...
	g.GET("/api/v1/conversations/spam", perm(handleGetSpamConversations, "conversations:read_all"))
	g.GET("/api/v1/conversations/trash", perm(handleGetTrashedConversations, "conversations:read_all"))
	...
	g.GET("/api/v1/conversations/by-ref/{ref}", perm(handleGetConversationByRef, "conversations:read"))
	...
	// /color before /{id} so the more specific path matches first.
	g.PUT("/api/v1/statuses/{id}/color", perm(handleUpdateStatusColor, "status:manage"))
	...
	g.GET("/api/v1/reports/activities", perm(handleGetRecentActivities, "reports:manage"))
