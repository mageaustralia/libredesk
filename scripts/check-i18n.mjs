...
// Matches: t('key'), $t("key"), i18n.t('key'), foo.t('key'), with optional
// leading/trailing whitespace inside the parens. The key char class is
// deliberately conservative — letters/digits/dots/underscores/dashes — so
// hardcoded sentences ("Hello world!") never look like keys.
...
function main () {
  ...
  for (const file of files) {
    ...
    while ((m = CALL_RE.exec(src)) !== null) {
      ...
      // with a variable, e.g. t('conversation.list.column.' + col) — the
      // actual lookup key is dynamic and out of scope for this check.
