...
import { linkSupport, link, linkBand, linkArrow, flowGroup, flowCircle, linkLabelGroup, linkLabelBackground, greyedOutLink, linkDashed, linkLabelContent } from './style.js';
...
function updateLinks(selection, config, duration, scale = 1, getLinkArrowDefId) {
    ...
    selection.each((d, i, elements) => {
        ...
        if (linkLabelText) {
            ...
            let linkLabelContent$1 = linkLabelGroup$1.select(`.${linkLabelContent}`);
            // If the label was hidden or didn't have text before, we need to set the initial position
            if (!linkLabelContent$1.size() || !linkLabelContent$1.text() || linkLabelContent$1.attr('hidden')) {
            ...
            linkLabelGroup$1.select(`.${linkLabelContent}`).remove();
            linkLabelContent$1 = linkLabelGroup$1
                .append(shouldRenderUseElement ? 'use' : 'text')
                .attr('class', linkLabelContent);
            const linkLabelFontSize = (_c = toPx(linkLabelDatum.fontSize)) !== null && _c !== void 0 ? _c : getCSSVariableValueInPixels('var(--vis-graph-link-label-font-size)', linkLabelContent$1.node());
            const linkLabelColor = (_d = linkLabelDatum.textColor) !== null && _d !== void 0 ? _d : getLinkLabelTextColor(linkLabelDatum);
