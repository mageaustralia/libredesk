        ...
        <template v-else-if="searchPerformed">
          ...
          <div v-if="hasMore" class="flex justify-center py-6">
            <Button variant="outline" :disabled="loading" @click="loadMore">
              ...
              {{ $t('search.loadMore', { shown: scopedShown, total: scopedTotal }) }}
            </Button>
          </div>
        </template>
...
// FS8: persist the last search across navigation (within the tab). The older
// fork format stored a flat array; discard anything that isn't the grouped
// shape so users upgrading mid-tab don't hit a broken iterator.
...
// FS14: the sidebar search icon clears sessionStorage and pushes a timestamped
// query while already on the search route. Clear in-memory state (hydrated from
// sessionStorage at mount) and refocus, since a same-route nav doesn't remount.
