...
const applyOpenScroll = () => {
  ...
  if (targetEl) {
    ...
    // FS24: land on the top of the last non-activity message rather than the
    // absolute bottom. Activity rows (status changes, assignments) tend to be
    // the newest entries and pinning them to the bottom hides the actual
    // thread on open. Falls back to scrollToBottom when there is no
    // non-activity message or its DOM node isn't rendered yet.
    ...
    const lastEl = lastMsg ? thread.querySelector(`[data-message-uuid="${lastMsg.uuid}"]`) : null
    if (lastEl) {
      hasUserScrolled.value = true
      scrollToOffset(Math.max(0, lastEl.offsetTop - 12))
    } else {
      hasUserScrolled.value = false
      scrollToBottom()
    }
