<script setup>
// Multi-select editor used inside a FilterPill popover. Two modes (Is any of
// / Is none of) map to backend operator pairs: `in` / `not_in` for ID-based
// fields, and `contains` / `not contains` for tags (the SQL builder is
// keyed on the operator name for tag-array semantics).
...
function initFromModelValue () {
  if (props.modelValue) {
    ...
    mode.value = op === excludeOp.value ? 'exclude' : 'include'
    try {
