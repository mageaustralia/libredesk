...
// GetAllForUser returns all macros with each row's per-user last_used_at
// populated for the given user. Sorted most-recently-used first (NULLs
// last), then alphabetically by name. Used by the agent-facing macro
// picker so each agent sees their own MRU order. RAG sync and other
// non-agent consumers should keep using GetAll.
...
// MarkUsed records that a user just applied a macro: bumps last_used_at and
// use_count via upsert. Called after a successful applyMacro so the picker
// re-sorts on the next fetch (frontend also re-sorts optimistically).
