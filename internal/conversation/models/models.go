...
type Conversation struct {
	...
	// merged_into_conversation_id — the reverse of MergedInto*. Populated
	// by a json_agg subquery in get-conversation, empty array when nothing
	...
	// direction without copy-pasting a ticket number from the activity log.
...
// Postgres json_agg yields a JSONB byte slice; sqlx/database/sql needs a
// Scanner on the target. Empty input (no merged secondaries) decodes to
// an empty, non-nil slice so the JSON response stays "[]" rather than
// "null" — keeps the frontend's `merged_from?.length > 0` checks cleanly
// true/false without null guards everywhere.
...
// conversation. MessageType discriminates the two so an agent can have
// a reply draft AND a private-note draft on the same conversation
// without them clobbering each other. Added in V2_2_20; legacy rows
// default to 'reply' (the old single-slot semantic).
...
// RecentActivity is one row in the global activity feed shown under
// Reports > Recent Activities. It joins conversation_messages to the parent
// conversation and the acting user so the frontend can render the timeline
// without follow-up requests. Total carries the COUNT(*) OVER() window value
// for the first row only and is omitted from JSON.
